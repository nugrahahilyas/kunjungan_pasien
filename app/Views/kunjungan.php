...
<?php dd($result); ?>
    ...
    <div class="container">
        <p class="text-center">
            <strong>
                Laporan Kunjungan Rawat Jalan per Kelurahan se Kabupaten <?= $data['array'][0]['kabupaten'] . ' Provinsi ' . $data['array'][0]['provinsi']; ?>
            </strong>
        </p>
...
<div class="row row-cols-2 d-flex flex-fill justify-content-around">
    <div class="col d-flex">
                        <div class="card m-2 float-end" style="width: 40rem;">
                            ...
                            <div class="card-body">
                                <table class="table">
                                    ...
                                    <?php foreach($data['arraydatatotalkunjungan'] as $kecamatan => $jumlah) : ?>
                                    <tr>
                                        <td><?= $kecamatan; ?></td>
                                        <td><?= $jumlah; ?></td>
                                    </tr>
                        ...
                        <?php foreach ($data['array'] as $perkecamatan => $value) : ?>
                            <?php if ($kecamatanTerakhir !== $perkecamatan) : ?>
                                <?php if ($kecamatanTerakhir !== '') : ?>
                                        <tr>
                                            <td colspan="2" class="text-center"><strong>Total</strong></td>
                                            <td><strong><?= $total_poliklinik; ?></strong></td>
                                            <td><strong><?= $total_igd; ?></strong></td>
                                            <td><strong><?= $total_kunjungan; ?></strong></td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
    ...
    ?>
    <?php endif; ?>
    <?php $i = 0; ?> <!-- Mulai ulang indeks untuk setiap kecamatan -->
    <!-- <div class="col-md-6"> Kolom setengah lebar -->
    <?php foreach($value as $va => $v) : ?>
        <!-- <?php dd($va); ?> --> <!-- Hapus baris ini -->
        <div class="card col d-flex flex-fill m-2" style="width: 40rem;">
            <div class="card-header alert-success" role="alert">
                <i class="fa-solid fa-location-dot" style="color: #1d5101;"></i> <?= $va; ?>
            </div>
            <div class="card-body">
                <table class="table m-2">
                    ...
                    <tbody>
                        ...
                        <?php 
                            // Mengakumulasi total dari setiap kolom
                            $total_poliklinik += $v; // Menggunakan $v bukan $perkecamatan
                            $total_igd += $v; // Menggunakan $v bukan $perkecamatan
                            $total_kunjungan += $v; // Menggunakan $v bukan $perkecamatan
                        ?>
                        <?php $kecamatanTerakhir = $perkecamatan; ?> <!-- Perbaiki kecamatanTerakhir -->
                        <?php $i++; ?>
                    <?php endforeach; ?>
                    <tr>
                        <td colspan="2" class="text-center"><strong>Total</strong></td>
